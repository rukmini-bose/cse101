...
int compare(BigInteger A, BigInteger B){
    ...
    // printf("I AM HERE IN THE BEGINNING\n");
    if (A == B) return 0;
    ...
    if(sign(A) < sign(B)){
        ...
        //printf("I am here because signs = 0\n");
        ...
        if(length(A->nnl) < length(B->nnl)){
            ...
            //printf("I am here because lengths are equal\n");
...
BigInteger stringToBigInteger(char* s){
    ...
    /* find the length of char* s */ 
    if(s == NULL){
        fprintf(stderr,"ERROR: cannot call stringToBigInteger() on an empty or NULL string array");
        exit(EXIT_FAILURE);
    } else{
        int len = strlen(s);
        /* create finalBig BigInteger and iterate from the back of char*s and check 
        if each of the elements of each iteration are part of base 10. 
        If so, add the back of char* s to the end of BigInteger 
        */
        int start = 0;
        ...
        for(i = len-1; i >= start; i -= POWER) { /* iterate through char* s*/
            int j = (i - POWER + 1);
            if (j < start) {
                break;
            }
...
BigInteger diff(BigInteger A, BigInteger B){
    ...
    if(A->sign == 1 && B->sign == -1){
        ...
        if(comparison == 1){
             printf("HELLO I AM HERE BECAUSE A > B\n");
             C = malloc(sizeof(BigIntegerObj));
             C->nnl = diffHelper(A->nnl, B->nnl);
             C->sign = -1;
             return C;
