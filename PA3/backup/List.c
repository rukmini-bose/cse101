...
int equalsList(List A, List B){
    ...
    if(A==NULL || B==NULL){
        fprintf(stderr, "ERROR: cannot call equals() on NULL list(s)\n");
        exit(EXIT_FAILURE);
    } else{ 
        equalsResult = (A -> length == B -> length);
        N = A->front;
        M = B->front;
        while(equalsResult && N != NULL){
            equalsResult = (N->data == M->data);
            N = N->next; 
            M = M->next;
        }
        return equalsResult;
...
void set(List A, long x){
    if(A == NULL){
        ...
        fprintf(stderr, "ERROR: cannot call set() on empty or invalid list");
        exit(EXIT_FAILURE);
    } else if(A->index < 0){
        fprintf(stderr, "ERROR: cannot can set() on list with negative index");
        exit(EXIT_FAILURE);
    }else{
        A->cursor->data = x;
